...
GLuint shaders::loadShaders(const char * vertex_file_path,const char * fragment_file_path){
	...
	_pr->Linking(vertex_file_path, fragment_file_path);
	return ProgramID;
}
...
void shaders::UseShaderInstanced(GLuint program, Camera* Cam, Vec3f l_Pos, Vec3f l_Int, float l_Ac, float l_At){
	...
	glUniformMatrix4fv(glGetUniformLocation(program, "Camera"), 1, GL_FALSE, &Cam->Matrix()[0][0]);
	glm::vec3 c = Cam->GetPosition();
	glUniform3f(glGetUniformLocation(program, "camPos"), c.x, c.y, c.z);
...
void shaders::UseTextShader(GLuint program, glm::mat4 p, glm::vec3 c){
	glUseProgram(program);
	glUniform3f(glGetUniformLocation(program, "textColor"), c.x, c.y, c.z);
	glUniformMatrix4fv(glGetUniformLocation(program, "projection"), 1, GL_FALSE, &p[0][0]);
}



void shaders::UseSBShader(GLuint program, Camera* cam){
	glUseProgram(program);

	glUniformMatrix4fv(glGetUniformLocation(program, "P"), 1, GL_FALSE, &cam->Projection()[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(program, "V"), 1, GL_FALSE, &cam->View()[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(program, "VP"), 1, GL_FALSE, &cam->GetViewProjection()[0][0]);
